...
dust=gadget.readsnap('/panfs/ds08/hopkins/phopkins/data2/RDI_dust_spectrum/HR/c.k05N4.HR/output/', 100, 3)
...
for i in range(9):
  ...
  for j in range(9):
    z_dust=pos_dust[:,2]-j*0.1 + 0.1

    radius=np.sqrt(x_dust*x_dust + z_dust*z_dust)
    id=np.where(radius <= 0.001)
    grainsize2=grainsize[id]
    X, H = np.histogram(grainsize2, bins = bins)
    print 9*i+j
    print H
    res[9*i+j,:] = H
print ''
